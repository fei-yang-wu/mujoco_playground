...
# ==============================================================================
"""Utilities for making wandb metric sections easier to scan."""
...
_EVAL_PERF_KEYS = frozenset(("walltime", "epoch_eval_time", "sps", "num_envs"))


def readable_wandb_metrics(metrics: Mapping[str, Any]) -> dict[str, Any]:
  """Returns a wandb-only view of metrics with clearer top-level sections.

  Brax eval metrics are naturally named under ``eval/...``.  Wandb uses the
  first slash-delimited component as the section name, so every reward
  component, error metric, termination flag, and eval summary value otherwise
  lands in one large ``eval`` section.

  This helper intentionally returns a copy so callers can keep local JSON,
  TensorBoard, comparison scripts, and console output on the canonical Brax
  names.
  """
  formatted = {}
