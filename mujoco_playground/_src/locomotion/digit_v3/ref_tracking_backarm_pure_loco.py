...
"""
In this environment, the robot is trained to track reference motions with a third arm attached to its torso.
The reference motions include the "walk" motion after picking up an object with three arms. 
Penalties on upper body (three arms) joint position/velocity are added to encourage the robot to keep the third arm steady during walking.
"""
...
from mujoco_playground._src.locomotion.digit_v3 import ref_loader_thirdarm_pure_loco as ref_loader
  ...
  return config_dict.create(
      ...
      obs_noise=config_dict.create(
          ...
          scales=config_dict.create(   # Reference https://www.science.org/doi/10.1126/scirobotics.adi9579
              ...
              # for ref noise
              ref_ee_pos=[0, 0.01], # m, additive, gaussian
      ...
      reward_config=config_dict.create(
         level=10, # not currently in use
          scales=config_dict.create(
              ...
              # tracking_torque = 0,#1,
              # Costs.
              root_motion_penalty=-0,
              gravity_proj_penalty=-0,
              action_rate=-2, #-3
              torque_penalty=-0.25, # -0.25
              joint_acc_penalty=-7e-6, # -7e-6
              upper_body_joint_vel_penalty = -0, # -1
              upper_body_joint_pos_penalty = -0, # -0.05
              foot_contact_penalty = -0,
      ...
      push_config=config_dict.create(
          enable=True,
          interval_range=[2, 15], # seconds
          magnitude_range=[0.1, 0.5], # m/s
      ),
  )
...
class DigitRefTracking_Loco(digit_base.DigitEnv):
  ...
  def __init__(
      self,
      task: str = "backarm_1foot_wholebody",
  ...
  def _post_init(self) -> None:
    ...
    self._left_foot_geom_id = self._mj_model.geom("left-foot").id
    self._right_foot_geom_id = self._mj_model.geom("right-foot").id
    ...
    self.kp = jp.array([
       ...
       90, 90, 120, 120, 30, 30, 60, 60, 60, 60,
       50, 50, 30, 30, #100, 200, 100, 100# 30, 50, 30, 30, # thirdarm
    ])
    self.kd = jp.array([
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # foot contact force
    lf_contact_force = collision.get_contact_force_between_geoms(data, self._left_foot_geom_id, self._floor_geom_id)
    rf_contact_force = collision.get_contact_force_between_geoms(data, self._right_foot_geom_id, self._floor_geom_id)

 
    # Initial local frame (call once at reset)
    pos_init_world = jp.array(data.qpos[:3]) # world position of torso at t=0
    quat_init_world = jp.array(data.qpos[3:7]) # w,x,y,z at t=0
    euler_init = self._quat2euler(quat_init_world, axes='rxyz') # Euler from initial quaternion (MuJoCo: w,x,y,z)
    yaw0 = euler_init[2]
    R_world_to_local = self._euler2mat(0, 0, -yaw0) # Rotation matrix: World -> heading-aligned local frame (yaw-only) (3,3)
    pos_init_local = jp.dot(R_world_to_local, pos_init_world) # Base position at t=0, expressed in that local frame
    R_world_to_body = (self._quat2mat(data.qpos[3:7])).T # Rotation matrix: world → body

    # Position in yaw-local frame
    pos_in_local = jp.dot(R_world_to_local, data.qpos[0:3]) # base position expressed in yaw-local
    pos_rel_in_local = pos_in_local - pos_init_local # base position relative to initial pose expressed in yaw-local
    quat_rel_to_init = self._quaternion_multiply(self._quaternion_inverse(quat_init_world), data.qpos[3:7]) # Orientation relative to initial, in quaternion
    euler_rel_to_init = self._quat2euler(quat_rel_to_init, axes='rxyz') # Orientation relative to initial, in Euler
    lin_vel_in_local = jp.dot(R_world_to_local, data.qvel[:3]) # base linear velocity expressed in local frame
    omega_world = jp.dot(R_world_to_body.T, data.qvel[3:6])
    ang_vel_in_local = jp.dot(R_world_to_local, omega_world) # base angular velocity expressed in local frame

    # base linear/angular velocity expressed in torso frame
    lin_vel_in_body = jp.dot(R_world_to_body, data.qvel[0:3]) # world -> torso
    ang_vel_in_body = data.qvel[3:6] 
    
    # EE positions
    ee_pos_world = data.xpos[self.ee_idx] # EE positions (global), shape (n_ee,3)
    ee_pos_rel_world = ee_pos_world - data.qpos[:3] # EE relative to base in world frame
    ee_pos_in_local = jp.dot(R_world_to_local, ee_pos_world.T).T # EE in yaw-local frame
    ee_pos_rel_in_local = (jp.dot(R_world_to_local, ee_pos_rel_world.T)).T # EE relative to base in yaw-local frame
    ee_pos_in_body = jp.dot(R_world_to_body, ee_pos_rel_world.T).T # EE relative to base in body frame
    ...
    info = {
        ...
        # init info
        "pos_init_world": pos_init_world,
        "quat_init_world": quat_init_world,
        "euler_init": euler_init,
        "R_world_to_local": R_world_to_local,
        "pos_init_local": pos_init_local,
        
        # state info
        "pos_in_local": pos_in_local,
        "pos_rel_in_local": pos_rel_in_local,
        "quat_rel_to_init": quat_rel_to_init,
        "euler_rel_to_init": euler_rel_to_init,
        "lin_vel_in_local": lin_vel_in_local,
        "ang_vel_in_local": ang_vel_in_local,
        "R_world_to_body": R_world_to_body,
        "lin_vel_in_body": lin_vel_in_body,
        "ang_vel_in_body": ang_vel_in_body,
        "ee_pos_world": ee_pos_world,
        "ee_pos_rel_world": ee_pos_rel_world,
        "ee_pos_in_local": ee_pos_in_local,
        "ee_pos_rel_in_local": ee_pos_rel_in_local,
        "ee_pos_in_body": ee_pos_in_body,
        ...
        "lf_contact_force": lf_contact_force,
        "rf_contact_force": rf_contact_force,
        
        # noisy state info
        "noisy_lin_vel_in_body": jp.zeros_like(data.qpos[0:3]),
        "noisy_ang_vel_in_body": jp.zeros_like(data.qvel[0:3]),
        ...
        "ref_idx": ref_idx,
        "task_label": 0.0,
        "ref_pos_in_local": jp.zeros_like(pos_in_local),
        "ref_pos_rel_in_local": jp.zeros_like(pos_rel_in_local),
        "ref_local_ori": jp.zeros_like(euler_rel_to_init),
        "ref_robot_lin_vel": jp.zeros_like(lin_vel_in_body),
        "ref_robot_ang_vel": jp.zeros_like(ang_vel_in_body),
        "ref_ee_pos_in_local": jp.zeros_like(ee_pos_in_local),
        "ref_ee_pos_rel_in_local": jp.zeros_like(ee_pos_in_local),
        "ref_ee_pos_in_body": jp.zeros_like(ee_pos_in_local),
        "ref_motor_joint_pos": jp.zeros(len(self.a_pos_idx)),
        "ref_motor_joint_vel": jp.zeros(len(self.a_pos_idx)),

        # noisy ref info
        "noisy_ref_ee_pos_in_local": jp.zeros_like(ee_pos_in_local),
        "noisy_ref_ee_pos_rel_in_local": jp.zeros_like(ee_pos_in_local),
        "noisy_ref_ee_pos_in_body": jp.zeros_like(ee_pos_in_local),
        ...
        "ref_ee_pos_in_local_future": jp.zeros([self.ref_future_len, 12]),
        "ref_ee_pos_rel_in_local_future": jp.zeros([self.ref_future_len, 12]),
        "ref_ee_pos_in_body_future": jp.zeros([self.ref_future_len, 12]),
        "ref_ee_pos_in_local_future_obs": jp.zeros([self.ref_future_obs_len, 12]),
        "ref_ee_pos_rel_in_local_future_obs": jp.zeros([self.ref_future_obs_len, 12]),
        "ref_ee_pos_in_body_future_obs": jp.zeros([self.ref_future_obs_len, 12]),
  ...
  def _update_root_state(self, info, data, rng):
    # states in yaw-local frame
    info["pos_in_local"] = jp.dot(info["R_world_to_local"], data.qpos[0:3])
    info["pos_rel_in_local"] = info["pos_in_local"] - info["pos_init_local"]
    info["quat_rel_to_init"] = self._quaternion_multiply(self._quaternion_inverse(info["quat_init_world"]), data.qpos[3:7])
    info["euler_rel_to_init"] = self._quat2euler(info["quat_rel_to_init"], axes='rxyz')
    info["lin_vel_in_local"] = jp.dot(info["R_world_to_local"], data.qvel[:3])
    omega_world = jp.dot(info["R_world_to_body"].T, data.qvel[3:6])
    info["ang_vel_in_local"] = jp.dot(info["R_world_to_local"], omega_world)
    # states in robot frame
    info["R_world_to_body"] = (self._quat2mat(data.qpos[3:7])).T
    info["lin_vel_in_body"] = jp.dot(info["R_world_to_body"], data.qvel[0:3])
    info["ang_vel_in_body"] = data.qvel[3:6]
    # EE positions
    info["ee_pos_world"] = data.xpos[self.ee_idx]
    info["ee_pos_rel_world"] = info["ee_pos_world"] - data.qpos[:3]
    info["ee_pos_in_local"] = jp.dot(info["R_world_to_local"], info["ee_pos_world"].T).T
    info["ee_pos_rel_in_local"] = jp.dot(info["R_world_to_local"], info["ee_pos_rel_world"].T).T
    info["ee_pos_in_body"] = jp.dot(info["R_world_to_body"], info["ee_pos_rel_world"].T).T

    # Foot contact force
    info["lf_contact_force"] = collision.get_contact_force_between_geoms(data, self._left_foot_geom_id, self._floor_geom_id)
    info["rf_contact_force"] = collision.get_contact_force_between_geoms(data, self._right_foot_geom_id, self._floor_geom_id)
    # jax.debug.print("LF contact force: {}, RF contact force: {}", info["lf_contact_force"], info["rf_contact_force"])
    
    # Noise values
    rng, key = jax.random.split(rng)
    info["noisy_lin_vel_in_body"] = info["lin_vel_in_body"] + jax.random.normal(key,(3,)) * self._config.obs_noise.scales.base_lin_vel[1] * info["noise_level"]
    rng, key = jax.random.split(rng)
    info["noisy_ang_vel_in_body"] = info["ang_vel_in_body"] + jax.random.normal(key,(3,)) * self._config.obs_noise.scales.base_ang_vel[1] * info["noise_level"]
    ...
    info["ref_pos_in_local"] = self.ref_loader.preloaded_refs["ref_pos_in_local"][info["ref_idx"]][info["step"]]
    info["ref_pos_rel_in_local"] = self.ref_loader.preloaded_refs["ref_pos_rel_in_local"][info["ref_idx"]][info["step"]]
    info["ref_euler_rel_to_init"] = self.ref_loader.preloaded_refs["ref_euler_rel_to_init"][info["ref_idx"]][info["step"]]
    info["ref_lin_vel_in_body"] = self.ref_loader.preloaded_refs["ref_lin_vel_in_body"][info["ref_idx"]][info["step"]]
    info["ref_ang_vel_in_body"] = self.ref_loader.preloaded_refs["ref_ang_vel_in_body"][info["ref_idx"]][info["step"]]
    info["ref_ee_pos_in_local"] = self.ref_loader.preloaded_refs["ref_ee_pos_in_local"][info["ref_idx"]][info["step"]]
    info["ref_ee_pos_rel_in_local"] = self.ref_loader.preloaded_refs["ref_ee_pos_rel_in_local"][info["ref_idx"]][info["step"]]
    info["ref_ee_pos_in_body"] = self.ref_loader.preloaded_refs["ref_ee_pos_in_body"][info["ref_idx"]][info["step"]]


    # Noise values
    rng, key = jax.random.split(rng)
    info["noisy_ref_ee_pos_in_local"] = self.ref_loader.preloaded_refs["ref_ee_pos_in_local"][info["ref_idx"]][info["step"]] + jax.random.normal(key,info["ref_ee_pos_in_local"].shape) * self._config.obs_noise.scales.ref_ee_pos[1] * info["noise_level"]
    rng, key = jax.random.split(rng)
    info["noisy_ref_ee_pos_rel_in_local"] = self.ref_loader.preloaded_refs["ref_ee_pos_rel_in_local"][info["ref_idx"]][info["step"]] + jax.random.normal(key,info["ref_ee_pos_rel_in_local"].shape) * self._config.obs_noise.scales.ref_ee_pos[1] * info["noise_level"]
    rng, key = jax.random.split(rng)
    info["noisy_ref_ee_pos_in_body"] = self.ref_loader.preloaded_refs["ref_ee_pos_in_body"][info["ref_idx"]][info["step"]] + jax.random.normal(key,info["ref_ee_pos_in_body"].shape) * self._config.obs_noise.scales.ref_ee_pos[1] * info["noise_level"]
  ...
  def _update_joint_state(self, info, data, rng):
    ...
    # Noise Values
    rng, key = jax.random.split(rng)
    info["noisy_motor_joint_pos"] = info["motor_joint_pos"] + jax.random.normal(key,(len(self.a_pos_idx),)) * self._config.obs_noise.scales.joint_pos[1] * info["noise_level"]
    rng, key = jax.random.split(rng)
    info["noisy_motor_joint_vel"] = info["motor_joint_vel"] + jax.random.normal(key,(len(self.a_pos_idx),)) * self._config.obs_noise.scales.joint_vel[1] * info["noise_level"]
  ...
  def _init_state_hist(self, info):
    ...
    info["lin_vel_hist"] = jp.tile(info["lin_vel_in_body"][None, :], [self.hist_len, 1])
    info["ang_vel_hist"] = jp.tile(info["ang_vel_in_body"][None, :], [self.hist_len, 1])
    ...
    info["noisy_lin_vel_hist"] = jp.tile(info["noisy_lin_vel_in_body"][None, :], [self.hist_len, 1])
    info["noisy_ang_vel_hist"] = jp.tile( info["noisy_ang_vel_in_body"][None, :], [self.hist_len, 1])
  ...
  def _update_state_hist(self, info):
    ...
    info["lin_vel_hist"] = jp.roll(info["lin_vel_hist"], shift=-1, axis=0).at[-1].set(info["lin_vel_in_body"])
    info["ang_vel_hist"] = jp.roll(info["ang_vel_hist"], shift=-1, axis=0).at[-1].set(info["ang_vel_in_body"])
    ...
    info["noisy_lin_vel_hist"] = jp.roll(info["noisy_lin_vel_hist"], shift=-1, axis=0).at[-1].set(info["noisy_lin_vel_in_body"])
    info["noisy_ang_vel_hist"] = jp.roll(info["noisy_ang_vel_hist"], shift=-1, axis=0).at[-1].set(info["noisy_ang_vel_in_body"])
  ...
  def _update_ref_future(self, info, reset_value: bool = False):
    ...
    info["ref_base_trans_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_pos_rel_in_local"][info["ref_idx"]][i])for i in future_idx])
    info["ref_base_ori_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_euler_rel_to_init"][info["ref_idx"]][i])for i in future_idx])
    ...
    info["ref_lin_vel_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_lin_vel_in_body"][info["ref_idx"]][i])for i in future_idx])
    info["ref_ang_vel_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_ang_vel_in_body"][info["ref_idx"]][i])for i in future_idx])
    info["ref_ee_pos_in_local_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_ee_pos_in_local"][info["ref_idx"]][i])for i in future_idx])
    info["ref_ee_pos_rel_in_local_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_ee_pos_rel_in_local"][info["ref_idx"]][i])for i in future_idx])
    info["ref_ee_pos_in_body_future"] = jp.stack([jp.ravel(self.ref_loader.preloaded_refs["ref_ee_pos_in_body"][info["ref_idx"]][i])for i in future_idx])
    ...
    info["ref_ee_pos_in_local_future_obs"] = jp.stack([info["ref_ee_pos_in_local_future"][i * self.ref_future_interval, :] for i in range(self.ref_future_obs_len)])
    info["ref_ee_pos_rel_in_local_future_obs"] = jp.stack([info["ref_ee_pos_rel_in_local_future"][i * self.ref_future_interval, :] for i in range(self.ref_future_obs_len)])
    info["ref_ee_pos_in_body_future_obs"] = jp.stack([info["ref_ee_pos_in_body_future"][i * self.ref_future_interval, :] for i in range(self.ref_future_obs_len)])
  ...
  def _update_done_info(self, done, info):
    ...
    """In some tasks, we use same reference trajectory per env """
  ...
  def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
    if self._config.is_noise:
        obs = jp.concatenate([
            ...
            # jp.ravel(info["noisy_ref_ee_pos_rel_in_local"]),
            jp.ravel(info["noisy_ref_ee_pos_in_body"]),
       ...
       obs = jp.concatenate([
            ...
            # jp.ravel(info["task_label"]),
            # jp.ravel(info["ref_ee_pos_rel_in_local"]),
            jp.ravel(info["ref_ee_pos_in_body"]),
            jp.ravel(info["act_hist_obs"][:10]),  # 20
        ])
       
    privileged_obs = jp.concatenate([
        # state info
        jp.ravel(info["pos_rel_in_local"]),
        jp.ravel(info["euler_rel_to_init"]),
        jp.ravel(info["lin_vel_in_body"]), # 3
        jp.ravel(info["ang_vel_in_body"]), # 3
        jp.ravel(info["gravity_proj"]),  # 3
        jp.ravel(info["qpos_hist_obs"][:10]),
        jp.ravel(info["motor_joint_vel"]),
        # jp.ravel(info["ee_pos_rel_in_local"]),
        jp.ravel(info["ee_pos_in_body"]), 
        # reference info
        ...
        # jp.ravel(info["ref_ee_pos_rel_in_local_future_obs"][0]),
        jp.ravel(info["ref_ee_pos_in_body_future_obs"][0]),
        # jp.ravel(info["task_label"]),
        # action
        jp.ravel(info["act_hist_obs"][:10]),  # 20
        # kp,kd
  ...
  def _get_reward(
    ...
    neg = {
        ...
        "upper_body_joint_vel_penalty": self._cost_body_joint_vel(
          info["motor_joint_vel"]
        ),
        "upper_body_joint_pos_penalty": self._cost_body_joint_pos(
          info["ref_motor_joint_pos"],
          info["motor_joint_pos"]
        ),
        "foot_contact_penalty": self._cost_foot_contact_force(
          info["lf_contact_force"],
          info["rf_contact_force"],
        ),
    }
    return pos, neg
  ...
  def _randomize_qpos_qvel(self, ref_idx, qpos_qvel_rng: jax.Array)-> jax.Array:
    ...
    qpos0 = qpos0.at[:3].set(
        self.ref_loader.preloaded_refs["ref_pos_in_local"][ref_idx][0]
    )
    qpos0 = qpos0.at[3:7].set(
        self.ref_loader.preloaded_refs["ref_quat_rel_to_init"][ref_idx][0]
    )
    ...
    qvel0 = qvel0.at[:3].set(
        self.ref_loader.preloaded_refs["ref_lin_vel_in_body"][ref_idx][0]
    )
    qvel0 = qvel0.at[3:6].set(
        self.ref_loader.preloaded_refs["ref_ang_vel_in_body"][ref_idx][0]
    )
  ...
  def _cost_body_joint_vel(
      self,
      motor_joint_vel: jax.Array,
  ) -> jax.Array:
    left_arm_error = jp.sum(jp.square(motor_joint_vel[6:10])) # left arm joints
    right_arm_error = jp.sum(jp.square(motor_joint_vel[16:20])) # right arm joints
    third_arm_error = jp.sum(jp.square(motor_joint_vel[20:24])) # neck arm joints
    return 0.1* (left_arm_error + right_arm_error + third_arm_error)  # Negative reward for upper body joint velocities
  
  def _cost_body_joint_pos(
      self,
      ref_motor_joint_pos: jax.Array,
      motor_joint_pos: jax.Array,
  ) -> jax.Array:
    left_arm_error = jp.sum(jp.square(motor_joint_pos[6:10] - ref_motor_joint_pos[6:10])) # left arm joints
    right_arm_error = jp.sum(jp.square(motor_joint_pos[16:20] - ref_motor_joint_pos[16:20])) # right arm joints
    third_arm_error = jp.sum(jp.square(motor_joint_pos[20:24] - ref_motor_joint_pos[20:24])) # neck arm joints
    return 10* (left_arm_error + right_arm_error + third_arm_error)  # Negative reward for upper body joint velocities

  def _cost_foot_contact_force(
    self,
    lf_contact_force: jax.Array,
    rf_contact_force: jax.Array,
  ) -> jax.Array:
    lf_mag = jp.linalg.norm(lf_contact_force)
    rf_mag = jp.linalg.norm(rf_contact_force)

    # Per-foot excess above threshold
    threshold = 150.0
    lf_excess = jp.maximum(lf_mag - threshold, 0.0)
    rf_excess = jp.maximum(rf_mag - threshold, 0.0)

    # Quadratic penalty on excess for stronger push against large spikes
    cost = lf_excess**2 + rf_excess**2
    return cost
