...
from mujoco_playground._src.locomotion.digit_v3 import ref_loader_with_motion_type as ref_loader
...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        hist_len=100,
        hist_interval=4,
        ref_future_len=10, #currently not important
        ref_future_interval=4,
        num_timesteps=0, 
        num_envs=1,
        is_rsi=False, # whether to use reference state initialization (RSI)
        rsi_length_ranges=[5, 10], # seconds, range of reference state initialization (RSI) segment length
        ...
        # Each list index = motion_id (0: locomotion, 1: manipulation,) 
        reward_config=config_dict.create(
            level=10, # not currently in use
            scales=config_dict.create(
                # Rewards.
                tracking_joint_pos=[3,3],
                # tracking_joint_vel=0,
                tracking_root_pos=[0,3],
                tracking_root_ori=[3,3],
                tracking_root_lin_vel=[3,1.5],
                tracking_root_ang_vel=[3,1.5],
                tracking_endeffector_pos=[3,3], 
                # tracking_torque = 0,#1,
                # Costs.
                root_motion_penalty=[-0,0], #-1.0,
                gravity_proj_penalty=[-0,0], # -2.0,
                action_rate=[-2,-4], #-3
                torque_penalty=[-0.25,-0.3], # -0.25
                joint_acc_penalty=[-7e-6,-10e-6], # -7e-6
            ),
            tracking_sigma=0.5,
        ),
...
class DigitRefTracking_Loco(digit_base.DigitEnv):
    ...
    def __init__(
        self,
        task: str = "thirdarm_wholebody",
        # task: str = "thirdarm_table",
    ...
    def _post_init(self) -> None:
        ...
        # === Convert reward scales to JAX arrays ===
        scales = dict(self._config.reward_config.scales)
        self.reward_scales = {
            k: jp.asarray(v) if isinstance(v, (list, tuple, np.ndarray)) else v
    ...
    def reset(self, rng: jax.Array) -> mjx_env.State:
        rng, ctrl_rng, push_rng, qpos_qvel_rng, ref_rng, act_rng, root_rng, joint_rng, gravity_rng, rsi_rng = (jax.random.split(rng, 10))
        ...
        if self._config.is_rsi:
            rsi_init_step, rsi_length = self._randomize_rsi(self.ref_loader.preloaded_refs["ref_motion_lens"][ref_idx], rsi_rng)
        else:
            rsi_init_step, rsi_length = 0, self.ref_loader.preloaded_refs["ref_motion_lens"][ref_idx]
            
    
        qpos0, qvel0 = self._randomize_qpos_qvel(ref_idx, rsi_init_step, qpos_qvel_rng)
    ...
    def rsi_reset(self, info, rng: jax.Array) -> mjx_env.State:
        rng, ctrl_rng, push_rng, qpos_qvel_rng, ref_rng, act_rng, root_rng, joint_rng, gravity_rng, rsi_rng = (jax.random.split(rng, 10))
        ...
        rsi_info = {
            ...
            "last_act": jp.zeros(len(self.a_pos_idx)),
            "last_last_act": jp.zeros(len(self.a_pos_idx)),
            "kp_scale": info["kp_scale"],
            "kd_scale": info["kd_scale"],
            "motor_strength_scale":info["motor_strength_scale"],
            "motor_targets": jp.zeros(len(self.a_pos_idx)),
        ...
        self._update_root_state(rsi_info, data, root_rng)
        self._update_joint_state(rsi_info, data, joint_rng)
        self._update_gravity(rsi_info, data, gravity_rng)
        # self._update_state_hist(info)
        self._init_state_hist(rsi_info)
        self._update_ref_future(rsi_info, reset_value=True)

        obs = self._get_obs(data, rsi_info)
        reward, done = jp.zeros(2)
        return mjx_env.State(data, obs, reward, done, metrics, rsi_info)
    ...
    def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
        ...
        motion_id = self.ref_loader.preloaded_refs["motion_types"][state.info["ref_idx"]]
        pos, neg = self._get_reward(data, state.info, state.metrics, done)
        pos = {k: v * self.reward_scales[k][motion_id] for k, v in pos.items()}
        # neg = {k: v * self._config.reward_config.scales[k] for k, v in neg.items()}
        neg = {k: state.info["noise_level"] * v * self.reward_scales[k][motion_id] for k, v in neg.items()}
        rewards = pos | neg
    ...
    def _update_root_state(self, info, data, rng):
        ...
        info["ref_base_local_pos"] = self.ref_loader.preloaded_refs["ref_base_local_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_base_local_trans"] = self.ref_loader.preloaded_refs["ref_base_local_trans"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_base_local_ori"] = self.ref_loader.preloaded_refs["ref_base_local_ori"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_base_robot_lin_vel"] = self.ref_loader.preloaded_refs["ref_base_robot_lin_vel"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_base_robot_ang_vel"] = self.ref_loader.preloaded_refs["ref_base_robot_ang_vel"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_local_ee_pos"] = self.ref_loader.preloaded_refs["ref_local_ee_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_base_local_ee_pos"] = self.ref_loader.preloaded_refs["ref_base_local_ee_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]

        # Noise values
        rng, key = jax.random.split(rng)
        info["noisy_ref_local_ee_pos"] = self.ref_loader.preloaded_refs["ref_local_ee_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]] + jax.random.normal(key,info["ref_local_ee_pos"].shape) * self._config.obs_noise.scales.ref_ee_pos[1] * info["noise_level"]
        rng, key = jax.random.split(rng)
        info["noisy_ref_base_local_ee_pos"] = self.ref_loader.preloaded_refs["ref_base_local_ee_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]] + + jax.random.normal(key,info["ref_base_local_ee_pos"].shape) * self._config.obs_noise.scales.ref_ee_pos[1] * info["noise_level"]
    ...
    def _update_joint_state(self, info, data, rng):
        ...
        # reference values (current goals, used for reward calculation)
        info["ref_motor_joint_pos"] = self.ref_loader.preloaded_refs["ref_motor_joint_pos"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
        info["ref_motor_joint_vel"] = self.ref_loader.preloaded_refs["ref_motor_joint_vel"][info["ref_idx"]][info["step"] + info["rsi_init_step"]]
    ...
    def _update_ref_future(self, info, reset_value: bool = False):
        ...
        future_idx = jp.minimum(info["step"] + info["rsi_init_step"] + future_offsets, max_step)
    ...
    def _get_termination(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
        ...
        ref_traj_end_condition = jp.where(
            self._config.is_rsi,
            info["step"] > info["rsi_length"],
            (info["step"] + self._config.ref_future_len) > ref_length,
        )
    ...
    def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
        if self._config.is_noise:
            obs = jp.concatenate([
                ...
                # jp.ravel(self.ref_loader.preloaded_refs["motion_types"][info["ref_idx"]]),  # 1
            ])
        else:
            obs = jp.concatenate([
                # info["base_local_trans"],
    ...
    def _randomize_qpos_qvel(self, ref_idx, rsi_init_step, qpos_qvel_rng: jax.Array)-> jax.Array:
        qpos_rng, qvel_rng = jax.random.split(qpos_qvel_rng, 2)
        
        qpos0 = self._init_q.copy()
        qpos0 = qpos0.at[:3].set(
            self.ref_loader.preloaded_refs["ref_base_local_pos"][ref_idx][rsi_init_step]
        )
        qpos0 = qpos0.at[3:7].set(
            self.ref_loader.preloaded_refs["ref_base_local_quat"][ref_idx][rsi_init_step]
        )
        qpos0 = qpos0.at[self.a_pos_idx].set(
            self.ref_loader.preloaded_refs["ref_motor_joint_pos"][ref_idx][rsi_init_step]
        )
        ...
        qvel0 = qvel0.at[:3].set(
            self.ref_loader.preloaded_refs["ref_base_robot_lin_vel"][ref_idx][rsi_init_step]
        )
        qvel0 = qvel0.at[3:6].set(
            self.ref_loader.preloaded_refs["ref_base_robot_ang_vel"][ref_idx][rsi_init_step]
        )
        qvel0 = qvel0.at[self.a_vel_idx].set(
            self.ref_loader.preloaded_refs["ref_motor_joint_vel"][ref_idx][rsi_init_step]
        )
        key, qvel_rng = jax.random.split(qvel_rng, 2)
        qvel0 = qvel0.at[:6].set(
            qvel0[:6] + jax.random.normal(key, (6,)) * self._config.obs_noise.scales.root_v_std
        )
        key, qvel_rng = jax.random.split(qvel_rng, 2)
        qvel0 = qvel0.at[self.a_vel_idx].set(
            qvel0[self.a_vel_idx] + jax.random.normal(key, (len(self.a_vel_idx),)) * self._config.obs_noise.scales.v_std
        )

        return qpos0, qvel0
  
    
    def _randomize_rsi(self, ref_motion_lens, rng):
        rng, key = jax.random.split(rng)
        length_s = jax.random.uniform(key, minval=self._config.rsi_length_ranges[0], maxval=self._config.rsi_length_ranges[1])
        rsi_length = length_s // self._ctrl_dt
        rsi_length = jp.clip(rsi_length, 1, ref_motion_lens - self._config.ref_future_len - 1)
        
        rng, key = jax.random.split(rng)
        rsi_init_step = jax.random.randint(key, (), 0, ref_motion_lens - self._config.ref_future_len - rsi_length - 1) # note: exclusive upper bound

        return rsi_init_step, rsi_length
