...
class DigitRefTracking_Loco(digit_base.DigitEnv):
  ...
  def __init__(
      self,
      task: str = "backarm_1foot_wholebody_caren",
  ...
  def _post_init(self) -> None:
    ...
    self.gear_ratios = self.gear_ratios[:24]
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # NEW: CAREN waveform params
    caren_amp_deg = 3.14/9
    caren_freq_hz = 0.25
    
    CAREN_targets =  jp.where(
       state.info["step"] < 1500, 
       0, 
       -caren_amp_deg * 0.5 * (1.0 - jax.numpy.cos(2.0 * jax.numpy.pi * caren_freq_hz * (state.info["step"]-1600)))
    )

    data = digit_dynamics.wholebody_thirdarm_caren_step(
        self.mjx_model, 
        state.data,   
        motor_targets,
        CAREN_targets,
  ...
  def _cost_torques(
      ...
      # return jp.sum(jp.square(torques[:20] / self.motor_torque_limit[:20])) + 3 * jp.sum(jp.square(torques[20:] / self.motor_torque_limit[20:]))
