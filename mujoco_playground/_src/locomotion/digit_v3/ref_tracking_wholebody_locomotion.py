      ...
      reward_config=config_dict.create(
         level=10, # not currently in use
          scales=config_dict.create(
              ...
              tracking_root_ori=3,
              tracking_root_lin_vel=3,
              tracking_root_ang_vel=3,
...
class DigitRefTracking_Loco(digit_base.DigitEnv):
  ...
  def __init__(
      self,
    #   task: str = "thirdarm_wholebody_roughterrain",
  ...
  def _get_obs(self, data: mjx.Data, info: dict[str, Any]) -> jax.Array:
    ...
    privileged_obs_no_dr = jp.concatenate([
        # state info
        jp.ravel(info["base_local_trans"]),
        jp.ravel(info["base_local_ori"]),
        jp.ravel(info["base_robot_lin_vel"]), # 3
    ...
    teacher_obs_compact = jp.concatenate([
        obs,
        jp.ravel(info["base_local_trans"]),
        jp.ravel(info["base_local_ori"]),
        jp.ravel(info["base_local_ee_pos"]),
    ...
    teacher_obs_refqpos = jp.concatenate([
        teacher_obs_compact,
        jp.ravel(info["ref_qpos_future_obs"][0]),
    ])

    teacher_obs_feedback = jp.concatenate([
        teacher_obs_refqpos,
        jp.ravel(info["base_robot_lin_vel"]),
        jp.ravel(info["base_robot_ang_vel"]),
        jp.ravel(info["gravity_proj"]),
        jp.ravel(info["motor_joint_vel"]),
    ])
    ...
    teacher_obs_error = jp.concatenate([
        obs,
        jp.ravel(info["ref_base_local_pos"] - info["base_local_pos"]),
        jp.ravel(_wrap_to_pi(info["ref_base_local_ori"] - info["base_local_ori"])),
        jp.ravel(info["ref_base_robot_lin_vel"] - info["base_robot_lin_vel"]),
        jp.ravel(info["ref_base_robot_ang_vel"] - info["base_robot_ang_vel"]),
        jp.ravel(info["ref_base_local_ee_pos"] - info["base_local_ee_pos"]),
        jp.ravel(info["ref_motor_joint_pos"] - info["motor_joint_pos"]),
    ])

    teacher_obs_error_vel = jp.concatenate([
        teacher_obs_error,
        jp.ravel(info["ref_motor_joint_vel"] - info["motor_joint_vel"]),
    ])

    return {
        "state": obs,
        "teacher_state_error": teacher_obs_error,
        "teacher_state_error_vel": teacher_obs_error_vel,
        "teacher_state_compact": teacher_obs_compact,
        "teacher_state_refqpos": teacher_obs_refqpos,
        "teacher_state_feedback": teacher_obs_feedback,
        "teacher_state": teacher_obs,
        "privileged_state_no_dr": privileged_obs_no_dr,
  ...
  def _randomize_qpos_qvel(self, qpos_qvel_rng: jax.Array)-> jax.Array:
