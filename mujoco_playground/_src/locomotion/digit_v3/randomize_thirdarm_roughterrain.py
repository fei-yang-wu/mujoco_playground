...
HFIELD_IMAGE_PATH = mjx_env.ROOT_PATH / "locomotion" / "digit_v3" / "xmls" / "assets"
...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand_dynamics(rng):
    ...
    rng, key = jax.random.split(rng)
    dmass = jax.random.uniform(key, minval=0, maxval=1)
    ...
    rng, key = jax.random.split(rng)
    dmass = jax.random.uniform(
        key, shape=(len(THIRD_ARM_IDS),), minval=0.5, maxval=2
    )
    ...
    hfield_dataset = []
    for fname in HFIELD_IMAGE_LIST:
        full_path = os.path.join(HFIELD_IMAGE_PATH, fname)
        img = imageio.imread(full_path)
        if img.ndim == 3:
          img = img[:, :, 0]
        resized_images = cv2.resize(
           img, 
           (int(model.hfield_nrow[0]), int(model.hfield_ncol[0])), 
           interpolation=cv2.INTER_LINEAR
        )
        img = resized_images.astype(np.float32).flatten() / 255.0  # Normalize to [0, 1]
        hfield_dataset.append(img)
    hfield_dataset = jp.array(hfield_dataset)
    
    hfield_idx = jax.random.randint(rng, shape=(), minval=0, maxval=len(HFIELD_IMAGE_LIST))
    hfield_data = hfield_dataset[hfield_idx]
