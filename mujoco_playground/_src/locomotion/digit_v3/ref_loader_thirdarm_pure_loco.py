...
class JaxReferenceLoader:
    ...
    def _preload_trajectories(self):
        ...
        preloaded_refs = {
            ...
            "ref_motor_joint_vel": [],
            "ref_pos_in_local": [],
            "ref_pos_rel_in_local": [],
            "ref_quat_rel_to_init": [],
            "ref_euler_rel_to_init": [],
            "ref_ee_pos_rel_in_local": [],
            "ref_lin_vel_in_body": [],
            "ref_ang_vel_in_body": [],
            # Uncomment if needed:
            "ref_ee_pos_in_body": [],
        ...
        for path in tqdm(npz_files, desc="Loading reference trajectories"):
            ...
            ref_ee_pos_in_local = jp.array(data["ee_pos_in_local"].reshape(-1, 5, 3)[::sampling_step])
            ref_ee_pos_in_body = jp.array(data["ee_pos_in_body"].reshape(-1, 5, 3)[::sampling_step])
            ...
            """
            ref_quat_rel_to_init = jp.array(ref_qpos[:, [6, 3, 4, 5]])  # (w, x, y, z) Notice: Specify the index for your reference trajectory
            ref_euler_rel_to_init = self.quat2euler(ref_quat_rel_to_init) #
            ref_pos_in_local = jp.array(ref_qpos[:, :3])
            ref_pos_rel_in_local = ref_pos_in_local - ref_pos_in_local[0]
            ref_ee_pos_rel_in_local = ref_ee_pos_in_local - ref_pos_in_local[:, None, :]

            ref_base_rot = jax.vmap(self.euler2mat)(ref_euler_rel_to_init)
            ref_base_rot = ref_base_rot.squeeze()
            # ref_lin_vel_in_body = jp.einsum('bij,bj->bi', ref_base_rot.transpose(0, 2, 1), ref_qvel[:, :3])
            # ref_ang_vel_in_body = jp.einsum('bij,bj->bi', ref_base_rot.transpose(0, 2, 1), ref_qvel[:, 3:6])
            ref_lin_vel_in_body = jp.array(ref_qvel[:, :3])
            ref_ang_vel_in_body = jp.array(ref_qvel[:, 3:6])
            ...
            preloaded_refs["ref_pos_in_local"].append(ref_pos_in_local)
            preloaded_refs["ref_pos_rel_in_local"].append(ref_pos_rel_in_local)
            preloaded_refs["ref_quat_rel_to_init"].append(ref_quat_rel_to_init)
            preloaded_refs["ref_euler_rel_to_init"].append(ref_euler_rel_to_init)
            preloaded_refs["ref_lin_vel_in_body"].append(ref_lin_vel_in_body)
            preloaded_refs["ref_ang_vel_in_body"].append(ref_ang_vel_in_body)
            preloaded_refs["ref_ee_pos_in_local"].append(ref_ee_pos_in_local)
            preloaded_refs["ref_ee_pos_rel_in_local"].append(ref_ee_pos_rel_in_local)
            preloaded_refs["ref_ee_pos_in_body"].append(ref_ee_pos_in_body)
