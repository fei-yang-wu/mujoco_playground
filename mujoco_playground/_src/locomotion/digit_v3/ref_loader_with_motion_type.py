...
"""
Dataset Organization & Motion Type Mapping
==========================================

Folder structure:
    ref_traj_dir/
    ├── <motion_type_A>/      # all trajectories of one type
    │   ├── traj1.npz
    │   ├── traj2.npz
    │   └── ...
    ├── <motion_type_B>/      # another type
    │   ├── traj1.npz
    │   └── ...
    └── (optional .npz files directly under ref_traj_dir → motion type 'default')

- Each **subfolder name** is a motion type label (you choose the names).
- Each `.npz` stores one trajectory with fields like qpos, qvel, ee_pos, true_length.
- Subfolder names must appear in the `self.motion_types` dictionary.
...
- Keys: your chosen motion type names (must match subfolder names).
- Values: integer IDs used during training (JAX-friendly).
- Integers let you select type-specific reward parameters, e.g.:

    weight = type_weights[preloaded_refs["motion_types"][i]]
...
Example:
    ...
    self.motion_types = {"walking": 0, "pick_place": 1, "default": 2}

    → preloaded_refs["motion_types"] = [0, 0, 1, 1]
"""
...
class JaxReferenceLoader:
    def __init__(self, ref_traj_dir, subsample_factor=1, device="cpu"):
        ...
        self.ref_data = None
        self.device = device
        self.ref_traj_dir = Path(ref_traj_dir)
    ...
    def _motion_type_id(self, name: str) -> int:
        if name in self.motion_types:
            return self.motion_types[name]
        raise ValueError(f"Unknown motion_type folder '{name}'. "
                        f"Known: {list(self.motion_types.keys())}")
    
    
    def _gather_npz_with_types(self):
        """
        Return list of (motion_type, npz_path).
        - If `ref_traj_dir` is a single .npz: motion_type = parent folder name or 'default'.
        - If directory:
            * .npz directly in root -> motion_type='default'
            * each immediate subfolder name is the motion_type; include its .npz files
        """
        entries = []

        if self.ref_traj_dir.is_file() and self.ref_traj_dir.suffix == ".npz":
            mt = self.ref_traj_dir.parent.name or "default"
            entries.append((mt, self.ref_traj_dir))
            return entries

        if not self.ref_traj_dir.exists():
            raise ValueError(f"Invalid ref_traj_dir: {self.ref_traj_dir}")

        if self.ref_traj_dir.is_dir():
            # root-level .npz -> default
            for p in sorted(self.ref_traj_dir.glob("*.npz")):
                entries.append(("default", p))
            # immediate subfolders -> motion types
            for sub in sorted([d for d in self.ref_traj_dir.iterdir() if d.is_dir()]):
                mt = sub.name
                for p in sorted(sub.glob("*.npz")):
                    entries.append((mt, p))

        if not entries:
            raise ValueError(f"No .npz files found under: {self.ref_traj_dir}")
    ...
    def _preload_trajectories(self):
        ...
        preloaded_refs = {
            ...
            # "ref_torque": [],
            # "task_label":[],
            # NEW: per-trajectory motion type (string array)
            "motion_types": [],
        }

        typed_files = self._gather_npz_with_types()
        sampling_step = self.subsample_factor

        for (motion_type, path) in tqdm(typed_files, desc="Loading reference trajectories"):
            data = np.load(path)
            ref_len = int(data["true_length"]) // sampling_step # int(data["true_length"])  # get original unpadded length
            ...
            preloaded_refs["ref_base_local_quat"].append(ref_base_local_quat)
            ...
            preloaded_refs["motion_types"].append(self._motion_type_id(motion_type))

        
        for key, lst in preloaded_refs.items():
                preloaded_refs[key] = jp.array(lst)
